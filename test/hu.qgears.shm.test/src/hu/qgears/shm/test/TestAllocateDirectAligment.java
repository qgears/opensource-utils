...
public class TestAllocateDirectAligment {
	...
	/**
	 * Verifies that direct memory, allocated by the JVM, is aligned to 16 
	 * bytes.
	 */
	@Test
	public void testAllocationAlignment() {
		...
		for (int i = 0; i < 10000; ++i) {
			INativeMemory mem = DefaultJavaNativeMemoryAllocator.getInstance()
					.allocateNativeMemory(r.nextInt(1009) + 1);
			mems.add(mem);
			PartNativeMemory pnm = new PartNativeMemory(mem, 0, 1);
			Assert.assertTrue(
					"Java allocated direct memory must be 16 byte aligned.",
					pnm.getNativePointer1() % 16 == 0);
		}
	}
}
