...
public class UtilString {
	...
	/**
	 * Strips blacklisted characters from the input {@link String}. Invalid
	 * characters are deleted. Returns a new {@link String} object which doesn't
	 * contain any invalid characters.
	 * 
	 * See {@link #isBlackListed(int)} and {@link #isControl(int)} for invalid
	 * characters.
	 * 
	 * @param text
	 * @return the text {@link String} without blacklisted characters
	 */
	public static String stripBlacklistedCharacters(String text){
		StringBuilder sb = new StringBuilder();
		for (int i=0; i<text.length(); i++) {
			int codePoint = text.codePointAt(i);
			if(!isBlackListed(codePoint)){
	...
	/**
	 * Returns <code>true</code> if the given Unicode codepoint is blacklisted.
	 * 
	 * Unicode control characters are considered blacklisted.
	 * 
	 * See {@link #isControl(int)}
	 * 
	 * @param codePoint
	 * @return
	 */
	private static boolean isBlackListed(int codePoint) {
	...
	/**
	 * Returns <code>true</code> if the supplied codepoint is a Unicode control
	 * character.
	 * 
	 * Control characters are: U+0000 - U+001F, U+007F, U+0080 - U+009F, U+2028
	 * and U+2029.
	 * 
	 * @param codePoint
	 * @return <code>true</code> if the given codepoint is a control character in Unicode
	 */
