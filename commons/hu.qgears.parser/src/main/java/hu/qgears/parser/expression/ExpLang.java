...
public class ExpLang {
	...
	public static String processInlineExpressionLanguage(String lang, boolean addAll) throws IOException {
		int idxS=lang.indexOf(begin);
		int idxE=lang.indexOf(end);
		String pre=lang.substring(0, idxS);
		String post=lang.substring(idxE+end.length());
		String exp=lang.substring(idxS+begin.length(), idxE);
		
		ExpLang el = ExpLang.parse(exp);
		el.addAll=addAll;
		String expLang = el.renderExpLang();

		return pre+expLang+post;
	}
		...
		while (line != null) {
			if (line.startsWith("@")) {
				...
				throw new IOException(
						"Line is not empty and is not a valid command: "
								+ lineNumber);
			}
			line = br.readLine();
			lineNumber++;
	...
	public ExpLang initDummy() {
		...
		addExp(
				3,
				new Exp(
						"id+((bracketOpen+(!|(expression+*1(comma+expression))))+bracketClose)",
						"fnCall"));
		return this;
	}
	...
	public String renderExpLang() {
		...
		for (Map.Entry<Integer, List<Exp>> entry : map.entrySet()) {
			...
			for (Exp e : entry.getValue()) {
				...
				String rule1 = rule.replaceAll(thisId, "("+levelStringThis+"|"+levelString+")");
...
//				}
	...
	void buildOrString(StringBuffer ret, List<String> li, StringBuffer pre,
			boolean addBracket) {
		if (li.size() > 0) {
			pre.append("(");
			ret.append("|");
